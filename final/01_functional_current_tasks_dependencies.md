...
2) Intended backend automation (event-driven)
...
2) Staff child rows (0..N):
- `idparent_task = <parent.idtask>`
- `idstaff = <staff uuid>`

3) Machinery child rows (0..N):
- `idparent_task = <parent.idtask>`
...
2) merge_key changes can “split” history
...
3) Calendar view uses created date, not always planned start
...
**Golf Example**: User creates "Aeration" for "North Course".
**Football Example**: User creates "Mowing" for "Training Pitch 1".
...
### A.2 Multi-site task
**Golf Example**: User selects "North Course" AND "South Course" for "Overseeding".
**Football Example**: User selects "Stadium Pitch" AND "Training Pitch 1" for "Fertilizing".
